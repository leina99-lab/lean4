# 『Mathematics in Lean』  Part 1: 기초 중의 기초
...
### 3.8 한 줄로 끝내는 `ring` — 대수 등식의 강력한 도구

`rw`를 하나씩 쓰는 방식은 원리를 이해하는 데 매우 좋다. 그러나 실제 Lean 4에는 대수적인 등식을 **한 번에 자동으로 해결**해 주는 `ring`이라는 강력한 전술이 있다.
...
`ring`은 이름 그대로 **환(Ring)** 구조에서의 등식 — 교환법칙, 결합법칙, 분배법칙 등 — 을 자동으로 계산하여 증명을 완료한다. 위 코드를 실행하면 InfoView에 `No goals`가 표시된다.
...
| `rw`에 쓸 수 있나? | 불가 (= 이 아니므로) | 가능 (동치이므로 바꿔치기 가능) |
...
```
theorem (정리)  ─── 이름 O, 재사용 O, "이것은 중요한 결과!"
lemma (보조정리) ─── 이름 O, 재사용 O, "이것은 보조 도구!"
