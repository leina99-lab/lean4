...
**예시 2**: "양의 실수 x, y에 대해 $x + y ≥ 2\sqrt{(xy)}$임을 증명하라"
...
- $x = \sqrt{2}$ (무리수)
- $y = \sqrt{2}$ (무리수)
- $x^y = \sqrt{2}^{\sqrt{2}}$ (가정에 의해 유리수)

조건을 만족한다! ✓

**경우 2**: $\sqrt{2}^{\sqrt{2}}$가 **무리수**인 경우

- $x = \sqrt{2}^{\sqrt{2}}$(가정에 의해 무리수)
- $y = \sqrt{2}$ (무리수)
...
2. 그러나 **어느 경우인지 모름**!
   - $\sqrt{2}^{\sqrt{2}}$가 유리수인지 무리수인지 이 증명에서는 결정 안 함
3. **구체적인 증인을 제시하지 않음**

**참고**: 실제로 $\sqrt{2}^{\sqrt{2}}$는 **무리수**임이 알려져 있다 (Gelfond-Schneider 정리, 1934년). 따라서 경우 2가 실제로 성립한다.
