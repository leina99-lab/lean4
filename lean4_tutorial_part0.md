...
> **목표**: Lean 4의 개념과 작동 원리를 이해한다  
> **다음 단계**: 기호나 키워드가 낯설면 `lean4_tutorial_dict.md`를 참조한다
...
**의미: "등호를 중심으로 왼쪽과 오른쪽이 거울에 반사된 것 처럼 서로 같다"**
...
## 1.4 `rfl` 완전 정복— "왼쪽에서 계산된 값이 있으면 그 값은 등호에 반사되어 오른쪽에 비쳐진 모양이됩니다. 당연한건 당연한겁니다. "
...
## 1.6 `#check`와 `#eval` — Lean에게 물어보기— "궁금한 건 못 참아!!!"
...
2. **수학의 3단계** — 정의 → 주장 → 증명
3. **Lean 명령어** — `def`, `theorem`, `#check`, `#eval`
4. **`rfl`** — "같은 건 같다" (계산해서 같으면 증명 완료)
5. **Type vs Prop** — 데이터의 세계 vs 명제의 세계
...
`n`이 구체적인 숫자가 아니기 때문이다. `n = 0`일 수도, `n = 100`일 수도 있어서 Lean은 `n + 0`을 구체적인 값으로 계산할 수 없다.
